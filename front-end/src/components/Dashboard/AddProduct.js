...
export default function ProductModal({ open, onClose, onSubmit, defaultValues }) {
  ...
  const { handleSubmit, control, reset, watch, setValue, register, formState: { errors } } = useForm({
    defaultValues: {
      title: "",
      description: "",
 ...
 useEffect(() => {
  ...
  {errors.category && <p style={styles.error}>{errors.category.message}</p>}
            ...
            <div style={styles.row}>
              ...
              <input type="number" {...register("price")} style={styles.inputSmall} />
              {errors.price && <p style={styles.error}>{errors.price.message}</p>}
            </div>

            <div style={styles.row}>
              <label style={styles.label}>Stock</label>
              <input type="number" {...register("stock")} style={styles.inputSmall} />
              ...
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 10 }}>
                {imagePreviews.map((img, index) => (
                  <div key={index} style={{ position: "relative" }}>
                    <img
                      src={typeof img === "string" ? `${process.env.REACT_APP_UPLOAD_URL}${img}` : URL.createObjectURL(img)}
                      alt={`preview-${index}`}
...
const styles = {
  ...
  modal: { background: "#fff", borderRadius: 10, width: 700, maxHeight: "80vh", overflow: "hidden", display: "flex", flexDirection: "column", padding: 20 },
  ...
  textarea: { flex: 1, padding: 10, height: 60, borderRadius: 6, border: "1px solid #ccc" },
  inputSmall: { flex: 1, padding: 8, borderRadius: 6, border: "1px solid #ccc" },
  subHeading: { fontSize: 16, fontWeight: "bold", marginTop: 10, marginBottom: -5 },
  button: { padding: "6px 12px", backgroundColor: "#e2e8f0", border: "none", borderRadius: 6, cursor: "pointer" },
  removeVariant: { padding: "4px 8px", backgroundColor: "#ef4444", color: "#fff", border: "none", borderRadius: 4, cursor: "pointer" },
  removeImage: { padding: "4px 8px", backgroundColor: "#ef4444", color: "#fff", border: "none", borderRadius: 4, cursor: "pointer" },
  previewImage: { width: 70, height: 70, objectFit: "cover", borderRadius: 6, border: "1px solid #ccc" },
